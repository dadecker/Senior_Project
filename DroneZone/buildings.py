...
# add airport coordinates to table only if within range
while(index < len(data['features'])):
	...
	if (inRange(mylat, mylon, buildinglat, buildinglon)):
		cursor.execute("INSERT INTO buildings(entry, airport, latitude, longitude, height) VALUES (%s, %s, %s, %s, %s);", (x, data['features'][index]['properties']['ARPT_Name'], data['features'][index]['properties']['LATITUDE'], data['features'][index]['properties']['LONGITUDE'], data['features'][index]['properties']['CEILING']))
		x+=1
		index+=1
	else:
		index+=1
